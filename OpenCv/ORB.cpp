...
void alignImages(Mat &object, Mat &scene, Mat &h)
{
	// Convert images to grayscale
	Mat objectGray, sceneGray;
	cvtColor(object, objectGray, CV_BGR2GRAY);
	...
	orb->detectAndCompute(objectGray, Mat(), keypointsObject, descriptorsObject);
	orb->detectAndCompute(sceneGray, Mat(), keypointsScene, descriptorsScene);

	// Match features.
	std::vector<DMatch> matches;
	Ptr<DescriptorMatcher> matcher = DescriptorMatcher::create("BruteForce-Hamming");
	matcher->match(descriptorsObject, descriptorsScene, matches, Mat());
	...
	for (int i = 0; i < good_matches.size(); i++)
	{
		...
		sc.push_back(keypointsScene[good_matches[i].trainIdx].pt);
	}

	for (int i = 0; i < obj.size(); i++)
	{
		cv::circle(img_matches, sc[i], 2, Scalar(0, 0, 255), FILLED, LINE_8);
	}

	h = findHomography(obj, sc, CV_RANSAC);
...
int mainorb(int argc, char **argv) try
{
	...
	Mat object = imread("plomb_min.jpg");

	while (true)
	{
		camera >> inputImage;

		cv::imshow("Camera", inputImage);
		...
		if (symbol == 's' || symbol == 'S')
		{
			...
			alignImages(object, inputImage, h);
